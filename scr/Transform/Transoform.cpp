...
void Transform::Rotate(float angle_in_degrees, Axies axie_to_roate)
{
	...
	{
	case AXIE_X:
		this->t_Rotate = glm::vec3(angle_in_degrees, t_Position.y, t_Position.z);
		break;
	case AXIE_Y:
		this->t_Rotate = glm::vec3(t_Position.x, angle_in_degrees, t_Position.z);
		break;
	case AXIE_Z:
		this->t_Rotate = glm::vec3(t_Position.x, t_Position.y, angle_in_degrees);
		break;
	default:
		break;
	}
...
void Transform::Scale(float amount, Axies axie_to_scale_along)
{
	...
	{
	case AXIE_X:
		this->t_Scale = glm::vec3(amount, t_Position.y, t_Position.z);
		break;
	case AXIE_Y:
		this->t_Scale = glm::vec3(t_Position.x, amount, t_Position.z);
		break;
	case AXIE_Z:
		this->t_Scale = glm::vec3(t_Position.x, t_Position.y, amount);
		break;
	default:
		break;
	}
